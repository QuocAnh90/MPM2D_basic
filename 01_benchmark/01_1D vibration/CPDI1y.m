...
%% Grid generation
NN(1,1)                 = 4;                               % number of nodes in X direction
NN(1,2)                 = 501;                               % number of nodes in Y direction
...
A_si                    = zeros(nodeCount*2,2);
    ...
    for sp=1: spCount
    e_sp(sp,2)               = 0.12 * (Length/2-x_sp(sp,2)) * exp(-60*(x_sp(sp,2)-Length/2).^2);
%     dp(p)               = 0.001 * exp(-60*(xp(p)-L/2).^2);
    s_sp(sp,2)               = e_sp(sp,2) * E;
    
    F_sp{sp}(2,2)               = F_sp{sp}(2,2) + e_sp(sp,2);
...
%     dp_exact(p)         = 0.0005 * (exp(-60*(xp(p)-L/2-sqrt(E)*ftime).^2)+exp(-60*(xp(p)-L/2+sqrt(E)*ftime).^2));
    ep_exact(sp,2)         = 0.0005 * ((-120*x_sp(sp,2)+120*sqrt(E)*ftime+60*Length)*exp(-60*x_sp(sp,2).^2+sqrt(E)*ftime*(120*x_sp(sp,2)-60*Length)+60*x_sp(sp,2)*Length-60*E*ftime*ftime-15*Length*Length)+(-120*x_sp(sp,2)-120*sqrt(E)*ftime+60*Length)*exp(-60*x_sp(sp,2).^2-sqrt(E)*ftime*(120*x_sp(sp,2)-60*Length)+60*x_sp(sp,2)*Length-60*E*ftime*ftime-15*Length*Length));
    sp_exact(sp,2)         = ep_exact(sp,2) * E;
    end
...
plot(x_sp(:,2),s_sp(:,2),'.',x_sp(:,2),sp_exact(:,2));

   title('Strain plot of MPM solution in 2D'); % title;
