...
%% Grid generation
NN(1,1)                 = 13;                               % number of nodes in X direction
...
fbc                     = [];                               % index of fixed nodes

fbn                     = 0           ;                  % magnitude of external forces
...
lp(1,1)                 =  le(1,1)/sqrt(particle_per_cell);                                 % size of particle in X direction
...
while sp<spCount+0.0001
    for i=1:24
        for j=1:6
            x_sp(sp,1:2)= [2*le(1,1)+0.5*lp(1,1)+(i-1)*lp(1,1) 8*le(1,2)+0.5*lp(1,2)+(j-1)*lp(1,2)];
            sp=sp+1;
        end
    end
end
...
writerObj1           = VideoWriter('CPDI1.avi');
...
nvelo_si(npid,:)                 = nvelo_si(npid,:) + nacc_middle_si(n,:)*dt;
...
caxis([0 0.25]);   
    ...
    frame1 = getframe(StressProfile1);
