...
%% Grid generation
resolution              = 2;
NN(1,1)                 = 12/(1/resolution)+1;               % number of nodes in X direction
...
while sp<spCount+0.0001
    for i=1:resolution*3
        for j=1:resolution*3
            x_sp(sp,1:2)= [5+0.5*lp(1,1)+(j-1)*lp(1,1) 9+0.5*lp(1,2)+(i-1)*lp(1,2)];
            sp=sp+1;
        end
    end  
end
...
writerObj2           = VideoWriter('CPDI1.avi');
